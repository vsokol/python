...
class Warehouse:
    ...
    def __init__(self, capacity, list_of_equipment):
        w_capacity = int(capacity)
        if isinstance(list_of_equipment, dict):
    ...
    def _remove_equipment(self, equipment, count):
        ...
        if vl is None:
            ...
            if count > vl:
                ...
                raise InvalidWarehouse(f"На складе недостаточно {equipment}. Запрошено {count}, доступно {vl}. Будет списано {vl}")
            else:
                self._list_of_equipment.update({equipment: count})
                self._number_of_occupied_places -= count
...
warehouse.consumption({Printer("Epson XP-600", 14, 39, 34, 5, 12, True): 3, Scanner("Canon CanoScan", 5, 25, 37, 2, 2400, 2400): 2})
print(warehouse.list_of_equipment)
print(f"warehouse = {warehouse}")